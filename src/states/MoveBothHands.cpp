#include "MoveBothHands.h"
#include "../JRLTask.h"
#include <mc_rtc/ConfigurationHelpers.h>

void MoveBothHands::start(mc_control::fsm::Controller & ctl) {
	auto & robot = ctl.robot();
	const auto & lh = robot.frame("LeftGripper");
	const auto & rh = robot.frame("RightGripper");
	...
	sva::PTransformd targetPoseRight(
			Eigen::Quaterniond(0.0, 0.7, 0.0, 0.7),
			Eigen::Vector3d(0.5, -0.25, 1.1)
		  );
	targetPoseRight.rotation() = Eigen::Quaterniond(0, 0.7, 0, 0.7).normalized().toRotationMatrix();
	rightHandTask_ = std::make_shared<mc_tasks::TransformTask>(rh, 5.0, 500.0);
	rightHandTask_->target(targetPoseRight);
	ctl.solver().addTask(rightHandTask_);
}

bool MoveBothHands::run(mc_control::fsm::Controller & ctl) {
...
void MoveBothHands::teardown(mc_control::fsm::Controller & ctl_)
{
	...
	mc_rtc::log::info("[MoveBothHands] teardown complete");
}

EXPORT_SINGLE_STATE("MoveBothHands", MoveBothHands)
